...
- 本地上传、拖动和缩放证件照
- 顶部联系方式按“电话 / 邮箱、出生年月 / 籍贯或所在地”两行两列显示，姓名和照片在右侧
...
1. 导入已有简历后，电话和邮箱位于左侧第一行；出生年月和籍贯 / 所在地位于第二行，姓名和照片位于右侧。
...
`birth`（出生年月，例如 `2001/08`）与 `location`（籍贯 / 所在地）均为选填文本字段，支持直接点击编辑。未填写时，编辑界面显示补填提示，打印 PDF 时隐藏空字段及其图标。JSON 中使用 `profile.birth`，Markdown / JSON 保存再导入均保留出生年月。旧版文件无需新增字段也能继续导入。
