...
3. 点击顶部“照片 → 上传”添加证件照。照片框默认约 26 × 35 mm，比旧版更大、更修长；右侧单独预留照片区，使用“照片”滑块在 40%–200% 范围内缩放时，左侧个人信息的列宽与位置保持不变。照片不与出生年月底部对齐，页首按实际照片高度留白，避免遮挡教育背景。裁切滑块调整图片缩放，照片框手柄调整位置；手动移动仍需自行避免重叠。
